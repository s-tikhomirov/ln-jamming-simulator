...
- set target node pairs (explicitly given or all adjacent to the target node)
- set honest senders and receivers
- opens jammer's channels (to explicitly given entry nodes or to all target node pairs' endpoints)
...
In jamming, we move to the next event when all target node pairs are jammed (as opposed to payment reaching the receiver once).
...
We use each routes multiple times, until all target node pairs are jammed.
When all target node pairs are jammed, or when no more hops can be jammed (i.e., there are no more routes), the jammer finishes the batch and pushes another event into the schedule.
...
We aim to touch as many yet unjammed target node pairs as possible with each next route to make jamming more efficient.
Routes may include different number of target node pairs (at least one).
We start from a maximum such number, specified as a parameter, for example 10.
We pick a subset of 10 unjammed target node pairs.
...
1. for the number N of target node pairs in each route from some maximum (think 10) decreasing to 1;
2. for an N-sized subset of yet unjammed target node pairs;
