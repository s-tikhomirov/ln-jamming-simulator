...
def test_get_channel_graph_from_json(example_snapshot_json):
	...
	assert(len(bc_edge_data) == 3 and
		"BCx0" in bc_edge_data and
		"BCx1" in bc_edge_data and
		"BCx2" in bc_edge_data)
	for dirX in [dir0, dir1]:
...
def test_get_routing_graph_from_json(example_snapshot_json):
	...
	bc_edge_data = g.get_edge_data(b, c)
	assert(len(bc_edge_data) == 2 and
		"BCx0" in bc_edge_data and
		"BCx1" in bc_edge_data)

	### Charlie - Bob
	assert(c in g.predecessors(b))
	cb_edge_data = g.get_edge_data(c, b)
	assert(len(cb_edge_data) == 3 and
		"BCx0" in cb_edge_data and
		"BCx1" in cb_edge_data and
		"BCx2" in cb_edge_data)
	...
	assert(not d in g.predecessors(c))
